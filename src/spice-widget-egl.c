...
gboolean spice_egl_init(SpiceDisplay *display, GError **err)
{
    ...
    if (spice_display_get_gl_scanout(SPICE_DISPLAY_CHANNEL(d->display)) != NULL) {
        SPICE_DISPLAY_DEBUG(display, "scanout present during egl init, updating widget");
...
void spice_egl_unrealize_display(SpiceDisplay *display)
{
    ...
    SPICE_DISPLAY_DEBUG(display, "egl unrealize %p", d->egl.surface);
...
void spice_egl_update_display(SpiceDisplay *display)
{
    ...
    SPICE_DISPLAY_DEBUG(display, "update %f +%d+%d %dx%d +%f+%f %fx%f", s, x, y, w, h,
                        tx, ty, tw, th);
                                  ...
                                  GError **err)
{
    ...
    attrs[11] = format;
    attrs[12] = EGL_NONE;
    SPICE_DISPLAY_DEBUG(display, "fd:%d stride:%u y0:%d %ux%u format:0x%x (%c%c%c%c)",
